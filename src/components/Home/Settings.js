      ...
      >
        {({
          ...
          <form onSubmit={handleSubmit}>
               {(urlImg1.length>0 && !isLoad  )&& <MDBBtn htmlFor="uploadImage"  className="loadPhoto" tag={(props)=> <label {...props} />} > 
           <img src={urlImg1} alt=""/>
           </MDBBtn> 
            }
            {(urlImg1.length>0 &&  isLoad  )&& <MDBBtn htmlFor="uploadImage"  className="loadPhoto" tag={(props)=> <label {...props} />} > 
           ...
           {(urlImg1.length<=0 && !isLoad) && <MDBBtn htmlFor="uploadImage"  className="loadPhoto" tag={(props)=> <label {...props} />} > 
            ...
            <div className={values.name.length>0? "form-group not-empty form-icon":"form-group form-icon"}> 
